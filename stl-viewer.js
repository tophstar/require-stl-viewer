define(['three'], function (THREE) {
	...
	{
		if(camera.position.z-(e.detail*10) < 100 || camera.position.z-(e.detail*10) > 600){
			return;
		}
		if(camera.position.z+(e.wheelDelta/2) < 100 || camera.position.z+(e.wheelDelta/2) > 600){
			return;
		}
		e.stopPropagation();
		if(e.wheelDelta)
			camera.position.z += e.wheelDelta/5;
		else
			camera.position.z -= e.detail;
		render();
	}
	function init()
	{
...
//		renderer.setClearColor( 0xffffff, 1);
	...
	function animate() {
		...
		//@todo make this an array OR ELSE...
		var animateInterval = setInterval(function(){
			//orgY = mesh.rotaiton.y;
			if(mesh.rotation.z < -0.3){
...
//			buildGeometry(oFile, f);
	...
	return function load(containerId)
	{
